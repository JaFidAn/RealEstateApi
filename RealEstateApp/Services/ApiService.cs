...
namespace RealEstateApp.Services
{
	public class ApiService
	{
		public static async Task<bool> RegisterUser(string name, string email, string password, string phone)
		{
			...
			var response = await httpClient.PostAsync(ApiSettings.ApiUrl + "api/users/register", content);
		...
		public static async Task<bool> Login(string email, string password)
		{
			...
			var content = new StringContent(json, Encoding.UTF8, "application/json");
			var response = await httpClient.PostAsync(ApiSettings.ApiUrl + "api/users/login", content);
			if (!response.IsSuccessStatusCode) return false;
